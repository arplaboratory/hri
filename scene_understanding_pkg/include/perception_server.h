...
class PerceptionServer
{
  ...
  void mocap_sr_position_callback(const geometry_msgs::Pose &msg);
  ...
  void pointcloud_bf_hololens_callback_any_camera(const scene_understanding_pkg_msgs::MeshPosArray &msg);

  void planner_start_assistive_guidance_callback(const std_msgs::Bool flag);
  void evaluate_obstacle_force_field(double dt);

  int traiangle_id_count = 1;
  ...
  ros::Subscriber hololens_cloud_in;
  ...
  ros::Publisher hololens_pc_latency;
  ros::Publisher drone_mesh_to_unity_time;
  ros::Publisher hololens_pointcloud;
  ros::Publisher hololens_pointcloud_in_left;
  ...
  tf::TransformBroadcaster mocap_sr_to_world;
  tf::TransformBroadcaster unity_frame_to_mocap_sr;
  tf::TransformBroadcaster unity_frame_to_hololens;
  ...
  void publish_obstacle_force_resultant(float fx, float fy, float Fm, Eigen::Vector2f pd, Eigen::Vector2f pd_dot);
  void publish_mocap_sim_tf();
   ...
   //Obstacle Force Field variables 
    ...
    Eigen::Vector2f p_distance_old;
    double t_stamp_old = 0.0;
    vector<geometry_msgs::Point> virtual_obstacles_vec;

   string mesh_vis_msgs_frame = "race10/map";
   string force_vis_msgs_frame = "mocap"; 
   string obstacle_force_function = "exponential";
